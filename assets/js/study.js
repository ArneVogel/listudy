...
function create_hand_shape(m, filled) {
    let ft = san_to_uci(chess, m.move);
    let shape = {orig: ft.to, brush: "normal"};
...
function display_arrows(once) {
    ...
    if (arrow_type == i18n.arrow_type_auto) {
        ...
        //if (total_moves < 10) {
            shapes.push(...clean_playable_moves.map(m => create_hand_shape(m, true)));
        //}
        ...
        //if (total_moves < 10) {
            shapes.push(...doubled_playable_move_objects.map(x => create_hand_shape(x.move, false)));
        //}
...
function main() {
//    overlay_manager.add_overlay(new TextOverlay("Outlined arrows\nare playable!", "d4", TextOverlayType.INFO, TextOverlayDuration.OneMove));;
    setup_ground();
    setup_chess();
    setup_trees();
